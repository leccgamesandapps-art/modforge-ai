...
function generateBedrock(prompt: string, name: string, namespace: string): ModFile[] {
  ...
  files.push({
    path: "README.md",
    type: "other",
    content: `# ${name}\n\nGenerated by **ModForge AI**\n\n> ${prompt}\n\n## Install (Bedrock)\n1. Open the .mcaddon file (not .zip)\n2. Minecraft should import BP + RP\n3. Enable both packs in world settings\n\n- Item: \`${itemId}\`\n- Block: \`${blockId}\`\n`,
  });
...
export async function createDownloadBlob(mod: GeneratedMod): Promise<{ blob: Blob; filename: string }> {
  ...
  // uint8array + octet-stream stops browsers renaming to .mcaddon.zip
  const bytes = await zip.generateAsync({ type: "uint8array", compression: "DEFLATE" });
  const blob = new Blob([bytes], { type: "application/octet-stream" });

  if (mod.platform === "java") {
    return { blob, filename: sanitizeFilename(mod.name, ".zip") };
  }
  return { blob, filename: sanitizeFilename(mod.name, ".mcaddon") };
}
